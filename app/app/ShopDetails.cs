...
namespace app
{
    public partial class ShopDetails : Form
    {
                    ...
                    if (dr == DialogResult.Yes)
                    {
                        if (dataGridView1.CurrentCell != null && dataGridView1.CurrentCell.Value != null)
                        {
                            ...
                            try
                            {
                                ...
                                var DueMemo = new DueMemo(dataGridView1[0, e.RowIndex].Value.ToString(), dataGridView1[2, e.RowIndex].Value.ToString(), dataGridView1[3, e.RowIndex].Value.ToString(), dataGridView1[13, e.RowIndex].Value.ToString(), dataGridView1[12, e.RowIndex].Value.ToString(), dataGridView1[11, e.RowIndex].Value.ToString());
                                DueMemo.ShowDialog();
                                DueMemo = null;
                                this.Show();
                                ReadData(sqlite_conn);
        ...
        public void ReadData(SQLiteConnection conn)
        {
            ...
            if (shopname != "All" && status == 0)
            {
                sqlite_cmd.CommandText = "SELECT * FROM ShopMemo WHERE ShopName='" + shopname + "' AND MemoDate LIKE '%" + memoDate + "%'";
            }
            // all shop and clear
            if (shopname == "All" && status == 1)
            {

                sqlite_cmd.CommandText = "SELECT * FROM ShopMemo WHERE MemoDate LIKE '%" + memoDate + "%' AND DueFlag='" + 0 + "' ";
            }

            // selected shop and clear
            if (shopname != "All" && status == 1)
            {
                sqlite_cmd.CommandText = "SELECT * FROM ShopMemo WHERE ShopName='" + shopname + "' AND MemoDate LIKE '%" + memoDate + "%' AND DueFlag='" + 0 + "' ";
            }
            ...
            if (shopname == "All" && status == 2)
            {
                ...
                sqlite_cmd.CommandText = "SELECT * FROM ShopMemo WHERE MemoDate LIKE '%" + memoDate + "%' AND DueFlag='" + 1 + "' ";
            }

            // selected shop and due
            if (shopname != "All" && status == 2)
            {
                sqlite_cmd.CommandText = "SELECT * FROM ShopMemo WHERE ShopName='" + shopname + "' AND MemoDate LIKE '%" + memoDate + "%' AND DueFlag='" + 1 + "' ";
            }
